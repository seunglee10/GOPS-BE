...
docker build -f infra/docker/Dockerfile.gops-backend -t "${ECR_BACKEND_REPO}:${IMAGE_TAG}" .
docker build -f infra/docker/Dockerfile.gops-frontend -t "${ECR_FRONTEND_REPO}:${IMAGE_TAG}" .
