...
> 사용하지 않는다. 모멘텀·균형·안정은 서버에서 변경하지 않는 시작 프리셋이며
> 사용자의 저장된 로직과 같은 프로필 목록에 섞지 않고, 선택 즉시 편집 가능한 미저장 초안으로 복제한다.
...
> 기술 key와 원문 뉴스 목록을 제외한 한국어 두 문장 근거만 표시하며, 응답이 오면 현재 편집 초안으로
...
2. 모멘텀·균형·안정 프리셋을 편집 가능한 새 로직의 시작점으로 불러오거나 AI 제안을 즉시 편집 초안으로 만들거나,
