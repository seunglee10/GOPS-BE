...
Any current-generation non-simulation order without `seed_profile` disables this seeded-account
path. Simulation orders are ignored for this decision, so entering SIM or placing a SIM order does
not replace the selected coach report. The fixed report is never written to Redis, ClickHouse,
Kafka, PostgreSQL, or S3.
...
The frontend normalization boundary removes legacy internal warning copy such as `DEV DEMO`,
`DEV FIXTURE`, and `fixed replay`; page 1 never renders a raw report warning as user-facing empty
state copy.
