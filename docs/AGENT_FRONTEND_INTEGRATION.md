...
생성하지 않으며 작은 화면에서는 하단 sheet로 전환한다. 상세 UI는 왼쪽 이벤트 강조선과
육각형 E/N 배지, 큰 제목, 날짜·세션 2열 정보, 테두리 없는 수치 행, 강조색 결과를 한 장의
세로형 카드에 배치한다. 뉴스 요약·원문과 실적 출처 같은 보조 정보는 카드 하단의 펼침
컨트롤 안에 두어 참고 디자인의 단순한 읽기 흐름을 유지한다.
