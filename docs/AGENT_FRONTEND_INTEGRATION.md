...
가격 라벨은 Canvas scene 좌표를 chart container의 local 좌표로 환산하고 계산된 가격축
영역 안에 배치해 UI scale·resize 중에도 점선과 분리되지 않게 한다. 라벨과 tooltip은
차트 체결 마커의 surface, border, typography 토큰을 재사용한다.
