...
패널 상단의 `추천 수식 설정`은 목록과 같은 레벨의 패널 내부 탭이다. 별도 페이지 제목 없이 프리셋부터 가중치 편집기까지 페이지 전체가 하나의 세로 스크롤 영역을 사용한다. overlay, dialog,
...
바꾸면 같은 그룹의 나머지 활성 값이 자동 재배분된다. 모멘텀·균형·안정은 서버에서 변경하지 않는
기본 프리셋이며 사용자 로직과 별도 선택 상태를 갖는다. 프리셋을 누르면 즉시 활성화하고, 비중을 처음 수정할 때만 이름과 비중을 편집할 수 있는 미저장 사용자 초안으로 전환한다. 저장된 사용자
로직은 현재 편집 상자 아래의 `저장된 로직` 목록에 표시한다. 사용자는 신호와 지표를 바로 추가·제거하고 slider와 숫자
입력으로 비중을 편집한다. 한 항목을 바꾸면 같은 그룹의
나머지 활성 항목을 비례 재배분해 합계 100%를 유지한다. 저장·활성화하면 종목 목록
탭으로 돌아가 새 profile revision으로 계산한 추천을 다시 읽는다.
저장된 추천 수식이 없는 초기 상태와 활성 사용자 수식 삭제 후에는 `안정` 프리셋을 기본 선택하되 `현재 선택한 로직` 상자는 표시하지 않는다.
사용자 로직이나 AI 초안을 편집하는 동안에는 세 프리셋을 모두 선택 해제한다. 선택한 기본 프리셋은 옅은 signal 배경과 파란 하단선으로 표시한다. 전체 비중 막대는
초기 파랑·초록·주황·분홍·보라·청록 신호 팔레트를 패널 배경과 섞은 저채도 색상으로 표시한다.
`내 로직`에는 자연어 요청 입력과 `AI 제안` 버튼을 둔다. AI 응답은 별도 `편집 중` 카드를 만들지 않고 즉시 `현재 선택한 로직`으로 전환한다. 검색 의도와 최신 evidence/news를 바탕으로 만든 한국어
두 문장 근거는 같은 상자 안의 정보 버튼 hover/focus에서만 표시한다. 원문 snapshot 수치, 기술 key, 뉴스 제목 목록은 노출하지 않는다. AI 응답은 저장·활성 프로필을 바꾸지 않으며, 기존
...
기사 뉴스의 `published_at`과 `localized_at`을 `virtualTime` 이하로 제한한 결과이며,
프런트가 가상시각을 query로 보내거나 live Redis 결과와 합치지 않는다. 일별 뉴스는
`generated_at <= virtualTime` 저장 스냅샷을
기본으로 사용하되, 서버가 최신 v2 요약의 모든 원문 `published_at`이 커서 이전임을 검증한
날짜는 `sourceMode=historical_reconstruction` summary로 대체할 수 있다. 미래 원문이
하나라도 포함된 날짜는 대체하지 않는다. 차트의 `GET /api/charts/events`는 계속
`generated_at <= virtualTime`인 저장 스냅샷만 반환한다. 추천 패널은 SIM 전 구간에서 기존 recommendation API를 다시 조회하고 서버가
...
요약은 등급 제목이나 상태색 없이 한 문장을 큰따옴표로 감싸 크게 표시한다. API가 이미
바깥 큰따옴표를 포함해도 화면에는 한 쌍만 표시한다. 확인 항목은 `차트`,
`뉴스`, `재무`, `시장` 순서의 2열 overview로 렌더링하고, 기본 화면에는 분류명, 상태,
최대 두 개 핵심 항목명만 크게 표시한다. 확인 항목과 차트 marker tooltip에는 출처나
ISO 기준시각을 표시하지 않는다. 차트 아래에는 캔들·오늘 경로·놓친 확인·미래 예측
여부를 설명하는 별도 범례를 두지 않는다.
...
`오늘의 추천 종목 보여줘` 명령은 서버 해석 호출 없이 프런트의 `market`
프리셋을 즉시 적용한다. 그 외 프리셋 이름과 화면 전환 표현이 함께 있는 요청은
기존 `layout.load` fast path를 사용한다. 이전 명칭인 `시장분석`,
