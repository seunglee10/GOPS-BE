...
시작 프리셋이며, 선택 즉시 이름과 비중을 편집할 수 있는 미저장 사용자 초안으로 전환한다. 저장된 사용자
...
저장된 추천 수식이 없는 초기 상태와 활성 사용자 수식 삭제 후에는 `안정` 프리셋을 편집 가능한 초안으로 기본 표시한다.
선택한 시작 프리셋은 옅은 signal 배경과 파란 하단선으로 표시한다. 전체 비중 막대는
