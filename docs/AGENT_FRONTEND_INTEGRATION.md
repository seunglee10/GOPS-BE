...
단, 활성 SIM run에서 서버가 `simulation-demo-score-profile.v1`로 반환한
`거래대금이 강하고 추세가 이어지는 종목` 제안은 `AI 제안` 성공 즉시 편집 초안에 적용한다.
사용자가 이어서 `저장하고 추천 재계산`을 누르면 NVDA 1위 시연 순위를 읽는다. 같은 문구라도
LIVE 또는 다른 promptVersion이면 기존 `초안에 적용` 확인 단계를 유지한다.
