...
- 첫 문장은 `제 의견은`으로 시작하는 결론 문장으로 쓴다.
- 본문은 `핵심 판단`, `판단 근거`, `반대로 볼 점`, `분석한 지표` 중
  필요한 2-3개만 사용한다.
- 사용자에게 추가 작업을 지시하는 checklist/to-do 섹션을 만들지 않는다.
  사용자에게 확인을 넘기지 말고, 확보한 자료만으로 핵심 판단을 구성한다.
- 마지막 보조 섹션은 `분석한 지표`로 두고 실제 사용한 자료/지표 종류만
  짧게 나열한다.
...
```text
제 의견은 개별 뉴스보다 시장/섹터 압력이 더 컸다는 것입니다.
