...
For the 20-symbol chart contract, use lazy historical expansion instead of preloading every candle up front. The `1D` baseline may be warmed first for previous-close, weekly/monthly, Watch List, and Hot Ranking responsiveness. `1m` should be fetched in reviewed bounded windows only when active charts or left-pan pagination need it. The v1 lazy `1m` lower bound is `BACKFILL_INITIAL_LOAD_1M_MIN_START=2020-07-01T00:00:00Z`, matching the six-year product target as of the 2026-07-01 stabilization. The operational default for `INITIAL_LOAD_INTERVALS` is `1D`; pass `INITIAL_LOAD_INTERVALS=1m` explicitly for intraday warm-cache windows. Processed candle output should use `S3_PROCESSED_FORMAT=parquet`, with historical raw chunks and processed compact manifests enabled. Existing queued/running/succeeded chunks are skipped during resume and do not consume `INITIAL_LOAD_MAX_ENQUEUE` capacity.
