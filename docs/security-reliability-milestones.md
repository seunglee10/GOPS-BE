...
이 문서는 [GOPS 통합 명세](./gops-integrated-spec.md)의 Kappa/Kafka-first 기준과 [주문 신뢰성/보안 상세 스펙](./spec.md)의 계약을 구현하고 검증할 순서를 정리한다. 목표는 MVP에서 KIS 모의투자 주문을 안전하게 수행하고, 실전 주문 전환 전에 중복 주문 방지, timeout 대사, 권한/한도, kill switch, 운영 알림을 검증하는 것이다.
...
- 주문 문서는 통합 스펙의 Kappa/Kafka-first 원칙, 상태, topic, MVP 범위, 보안 원칙을 따른다.
- Kafka는 주문 command와 결과 이벤트의 event spine이다.
- PostgreSQL은 idempotency, transaction guard, append-only 원장, transactional outbox, projection을 담당한다.
...
sequenceDiagram
  ...
  participant P as Broker Event Listener/Reconciler
  ...
  KIS-->>A: 성공/거부/timeout
  A->>DB: SUBMITTED/REJECTED/SUBMIT_FAILED_UNKNOWN + result outbox 저장
  DB->>K: broker.submit-results.v1 발행
  KIS-->>P: broker 주문/체결 event
  P->>KIS: timeout/open/unknown만 제한 조회
  P->>DB: PARTIALLY_FILLED/FILLED/CANCELED/RECONCILIATION_REQUIRED 반영
  P->>K: broker.order-events.v1 발행
  K->>R: result/order event replay
  R->>W: order_id 기준 상태 변경
...
Kappa/Kafka-first 원칙, 주문 상태, topic, 책임 경계를 고정해 이후 구현이 문서별로 흔들리지 않게 한다.

### 구현 범위

- 통합 스펙에 Kafka-first는 Kafka-only가 아니라는 기준을 명시한다.
...
- Backend API, KIS Broker Adapter, Broker Event Listener/Reconciler, Read Model Projector, WebSocket Gateway, PostgreSQL, Kafka의 책임을 문서화한다.
- MVP는 KIS 모의투자 주문, 실전 주문은 후속 전환이라는 범위를 명확히 둔다.

### 완료 기준

- 문서 간 주문 상태와 topic 명칭이 일치한다.
- 별도 주문 확정 topic이나 폐기된 상태명이 남아 있지 않다.
- PostgreSQL은 Kafka를 대체하는 이벤트 로그가 아니라 transaction guard/projection으로 설명된다.
...
KIS 제출 결과를 PostgreSQL transaction으로 남기고 `broker.submit-results.v1`로 발행해 read model과 WebSocket이 Kafka 결과 이벤트 기준으로 갱신되게 한다.
...
- Result Outbox Publisher는 `broker.submit-results.v1` 발행 성공 후 `published_at`을 기록한다.
...
- `broker.submit-results.v1` replay로 제출 결과 read model을 복구할 수 있다.
...
- Read Model Projector 종료 후 replay.

## M5. Broker Event Listener와 제한된 대사

### 목표

KIS 주문 제출 응답만 믿지 않고 broker 주문/체결 event와 제한된 주문/체결내역 조회로 최종 상태를 확인한다. 특히 `SUBMIT_FAILED_UNKNOWN`을 안전하게 해소하되, 전체 주문 상시 polling으로 broker API를 압박하지 않는다.

### 구현 범위

- broker가 제공하는 주문/체결 event channel이 있으면 primary path로 수신한다.
- event 누락, timeout, open order, 장기 미종결 주문만 bounded reconciliation 대상으로 삼는다.
- 제한된 조회 결과를 `broker.order-events.v1`로 발행한다.
...
- 계좌별 rate budget, adaptive backoff, jitter, circuit breaker를 적용한다.

### 완료 기준

- timeout 주문이 KIS 조회에서 발견되면 내부 상태가 보정된다.
- 완료/거부/취소된 terminal 주문은 polling 대상에서 제외된다.
- 대사 조회량은 open/unknown 주문 수와 계좌별 rate budget 안에서 제한된다.
- KIS에는 있는데 내부에 없는 주문은 운영 알림 대상이 된다.
- 내부에는 있는데 KIS에 없는 주문은 재조회 후에도 없을 때 수동 확인 상태로 남긴다.
- `broker.order-events.v1` replay로 체결/취소 read model을 복구할 수 있다.
- 대사 결과는 `order_id` 기준 WebSocket 상태 push로 이어진다.

### 장애 테스트

- Broker Event Listener/Reconciler 종료 후 재시작.
- KIS 조회 API 지연 또는 실패.
- 부분 체결 후 전량 체결 이벤트 순차 반영.
- 내부 수량/가격과 KIS 조회 수량/가격 불일치.
- Read Model Projector 종료 후 replay.
- rate limit 초과에 따른 backoff와 circuit breaker 동작.
...
- Read Model Projector 지연으로 stale 상태가 표시되는 경우.
...
- metric: API latency, idempotency conflict count, Kafka publish latency, consumer lag, projection lag, KIS timeout count, KIS reject count, DLQ count, outbox unpublished count, reconciliation mismatch count.
- alert: `SUBMIT_FAILED_UNKNOWN` 장기 지속, `RECONCILIATION_REQUIRED`, DLQ 급증, KIS timeout 급증, outbox 미발행 누적, consumer lag 급증, projection lag 급증, circuit breaker open.
...
- consumer lag 또는 projection lag 급증.
- redaction 누락 테스트.
...
Frontend server, Backend API, Kafka, KIS Adapter, Broker Event Listener/Reconciler, Outbox Publisher, Read Model Projector, PostgreSQL 중 일부가 죽어도 주문 상태가 유실되지 않고 최종적으로 수렴하는지 검증한다.
...
| Broker Event Listener/Reconciler down | broker event 재수신 또는 제한된 대사로 상태를 따라잡는다. |
...
- 복구 후 사용자 화면, read model, DB 최신 상태가 일치한다.
...
6. M5 Broker Event Listener/Reconciler와 `broker.order-events.v1` 발행을 구현한다.
...
- broker event 또는 제한된 KIS 주문/체결내역 대사로 `SUBMIT_FAILED_UNKNOWN`을 해소할 수 있다.
- Outbox Publisher 장애 후에도 command와 결과 이벤트가 유실되지 않는다.
- `broker.submit-results.v1`, `broker.order-events.v1` replay로 read model을 복구할 수 있다.
