...
sequenceDiagram
    ...
    FE->>BE: polling 또는 WebSocket/SSE 상태 조회
...
- 주문 상태 전이를 `RECEIVED -> SUBMITTING -> SUBMITTED/REJECTED/SUBMIT_FAILED_UNKNOWN -> PARTIALLY_FILLED/FILLED/CANCELED/RECONCILIATION_REQUIRED`로 문서화한다.
...
중복 클릭, 새로고침, 네트워크 재시도 상황에서도 같은 주문 의도가 여러 실전 주문으로 제출되지 않게 한다.
...
- `202 Accepted` 응답을 받으면 주문 상세 화면에서 상태 polling 또는 WebSocket/SSE 구독을 시작한다.
...
- 인증/인가, 계좌 접근 권한, 주문 가능 한도, market, symbol, qty, price를 검증한다.
...
- timeout/connection reset은 `SUBMIT_FAILED_UNKNOWN`으로 저장하고 즉시 재POST하지 않는다.
...
- frontend는 polling, WebSocket, SSE 중 하나로 상태를 갱신한다.
...
- WebSocket/SSE 끊김 후 polling fallback.
...
9. frontend 주문 상태 조회 API를 붙인다.
10. 주문 내역 화면을 만든다.
11. 보유 종목, 주문 가능 금액, 현금/잔고 화면과 연결한다.
12. timeout, 중복 클릭, 서버 재시작, Kafka 재처리 테스트를 만든다.
13. 실전 주문용 kill switch, 한도, circuit breaker, 운영 알림을 붙인다.
