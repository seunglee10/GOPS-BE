...
`infra/k8s/overlays/aws/scheduled/cronjob-order-flow-daily-rollup.yaml`. The shared dev
`aws-incluster-app-ci` deploy path also includes that scheduled CronJob via the
in-cluster app overlay, so both deploy paths consume one manifest. Ad hoc
