...
## 12-A. ClickHouse 개발/운영 기준

개발 환경에서는 ClickHouse를 로컬 Docker 환경에서 실행한다. 로컬 ClickHouse는 별도 사용료가 발생하지 않으며, 과거 캔들 조회 SQL과 Chart API 응답 검증에 사용한다.

| 사용 방식 | ClickHouse 비용 | 실제 비용 |
|---|---:|---|
| 로컬 PC Docker | 없음 | 내 PC 자원 사용 |
| 개인 서버/EC2 직접 설치 | 없음 | 서버 비용, 디스크 비용 |
| Kubernetes에 직접 설치 | 없음 | 노드 비용, 스토리지 비용 |
| ClickHouse Cloud | 있음 | 사용량 기반 과금 |

운영 환경에서는 ClickHouse를 직접 서버에 설치하거나 ClickHouse Cloud를 사용할 수 있다. 직접 설치 시 ClickHouse 소프트웨어 비용은 없지만 서버, 디스크, 백업, 운영 비용이 발생한다.

우리 구조에서 역할은 다음과 같이 나눈다.

```text
S3 = 장기 보관
ClickHouse = 빠른 과거 조회/분석
Redis = 최신값과 실시간 렌더링 캐시
```

운영에서는 최근 자주 조회하는 데이터는 ClickHouse에 적재하고, 전체 장기 데이터는 S3에 Parquet으로 저장한다.
...
사용자가 직접 이해하고 실행할 폴더는 아래처럼 단계명으로 분리한다.

```text
프론트 화면/      # 회사명/심볼 입력, Alpaca 신청 JSON, Redis/S3 확인 경로 표시
알파카 송신/      # 회사명/심볼을 Alpaca 구독 요청서로 변환
카프카 수신/      # Alpaca WebSocket에서 받은 원본을 Kafka Raw Topic에 저장
카프카 송신/      # Kafka Raw Topic을 스트리밍 처리 단계로 전달
레디스 확인/      # Redis 최신값 확인
S3 확인/          # S3 장기 저장 파일 확인
```

```mermaid
flowchart LR
  FE["프론트 화면<br/>회사명/심볼 입력"] --> A["알파카 송신<br/>구독 요청서 생성"]
  A --> B["카프카 수신<br/>Alpaca 원본 수신"]
  B --> C["Kafka Raw Topic"]
  C --> D["카프카 송신<br/>Flink 처리 단계"]
  D --> E["market.ticks.v1"]
  D --> F["market.candles.live.1m.v1"]
  D --> G["market.candles.closed.v1"]
  E --> R["Redis 최신값"]
  F --> R
  G --> R
  E --> S["S3 장기 저장"]
...
이동평균선은 `카프카 수신`에서 계산하지 않는다. `카프카 송신` 이후 스트리밍 처리 단계에서 candle 데이터의 `ma` 필드에 포함한다.
