...
| `gops-market-processor` | `infra/docker/Dockerfile.gops-market-processor` | `systems/market-data/pods/market-processor`, `systems/market-data/jobs/symbol-registry-sync`, `systems/market-data/jobs/coverage-repair`, `systems/market-data/jobs/candle-bootstrap`, `systems/market-data/shared` | market-processor pod, symbol-registry-sync job, coverage-repair job, operator-run candle bootstrap job |
| `gops-market-storage` | `infra/docker/Dockerfile.gops-market-storage` | `s3-sink`, `clickhouse-loader`, news rebuild jobs, `systems/fundamentals`, market shared code | processed S3 sink, raw S3 archive, clickhouse-loader pods, news rebuild jobs, and SEC companyfacts backfill job |
...
- `gops-market-storage` groups processed S3 sink, raw S3 archive, ClickHouse loader, news rebuilds, and SEC companyfacts backfill because they are storage/projection workers that share S3, ClickHouse, Redis, and market shared helpers.
