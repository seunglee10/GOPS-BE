...
| `gops-market-processor` | `infra/docker/Dockerfile.gops-market-processor` | `systems/market-data/pods/market-processor`, `systems/market-data/jobs/symbol-registry-sync`, `systems/market-data/shared` | market-processor pod, symbol-registry-sync job |
| `gops-market-storage` | `infra/docker/Dockerfile.gops-market-storage` | `clickhouse-loader`, market shared code | clickhouse-loader pod with optional post-insert S3 archive |
...
- `gops-market-storage` is centered on ClickHouse loading. Runtime S3 archive happens inside the loader only after ClickHouse accepts rows.
