...
| `gops-market-storage` | `infra/docker/Dockerfile.gops-market-storage` | `s3-sink`, `clickhouse-loader`, market shared code | processed S3 sink, backup-only raw S3 archive, and clickhouse-loader pods |
...
- `gops-market-storage` groups processed S3 sink, backup-only raw S3 archive, and ClickHouse loader because they are market storage workers.
