...
This document describes the repository contract and the live dev EKS Kafka
topology observed on 2026-07-10. The live cluster has 42 application topics plus
...
flowchart LR
  subgraph Clients["Kafka clients in namespace alfaka-market-data"]
    ...
    Consumers["Consumers<br/>Market processors<br/>Storage loaders<br/>Agent/order/news/chart workers"]
...
flowchart LR
  ...
  RawArchive["alfaka-raw-s3-archive<br/>group · 1 pod<br/>auto commit default"]
  ...
  subgraph Layer["Canonical layer topics"]
    ...
    LL["market.layer.candles.live.v1<br/>3P · RF1 · 30m"]
...
`market.layer.candles.live.v1` is produced for live/debug fanout but has no
current storage consumer. Redis is the active live chart state. The processed
S3 sink stores only interval closed candles and events. Trades and quotes are
archived through raw S3 and loaded into ClickHouse by the tick loader.

## News, Chart Derived Data, And Alerts
...
flowchart LR
  ...
  Backend["Backend chart API"]
  ChartRequest["market.chart-derived.requests.v1<br/>3P · RF1 · 7d"]
  ChartWorker["chart-derived worker<br/>group · 2 pods · manual commit"]
  ChartDLQ["market.chart-derived.dlq.v1<br/>3P · RF1 · 7d<br/>no current consumer"]
  ...
  Backend --> ChartRequest --> ChartWorker
  ChartWorker --> ClickHouse
  ChartWorker --> Redis
  ChartWorker -->|"failure"| ChartDLQ
...
| 13 | `market.layer.candles.live.v1` | 3 | 30m | market processor | none |
| 14 | `market.layer.candles.closed.v1` | 3 | 7d | legacy compatibility | none |
| 15 | `market.layer.candles.1m.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 16 | `market.layer.candles.5m.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 17 | `market.layer.candles.10m.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 18 | `market.layer.candles.1h.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 19 | `market.layer.candles.4h.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 20 | `market.layer.candles.1d.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 21 | `market.layer.candles.1w.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 22 | `market.layer.candles.1mo.closed.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 23 | `market.layer.trades.v1` | 3 | 2h | market processor | ClickHouse tick loader, event detector, alert evaluator |
| 24 | `market.layer.quotes.v1` | 3 | 2h | quote processor | ClickHouse tick loader |
| 25 | `market.layer.events.v1` | 3 | 7d | market processor | ClickHouse, processed S3, event detector |
| 26 | `market.news.alpaca.v1` | 3 | 7d | news ingestor and recent backfill | ClickHouse loader, news intelligence |
| 27 | `market.news.daily-summary-dirty.v1` | 3 | 7d | news intelligence | news daily summary worker |
| 28 | `market.chart-derived.requests.v1` | 3 | 7d | Backend chart service | chart-derived worker |
| 29 | `market.chart-derived.dlq.v1` | 3 | 7d | chart-derived worker | none |
| 30 | `orders.commands.v1` | 3 | 7d | order outbox publisher | `kis-broker-adapter` |
| 31 | `broker.submit-results.v1` | 3 | 7d | order outbox publisher | none |
| 32 | `broker.order-events.v1` | 3 | 7d | order outbox publisher | none |
| 33 | `orders.dlq.v1` | 3 | 7d | reserved; active DLQ is Postgres | none |
| 34 | `alerts.triggered.v1` | 3 | 7d | alert evaluator outbox sender | none; audit/replay topic |
| 35 | `alerts.dlq.v1` | 3 | 7d | alert evaluator | none |
| 36 | `agents.market-events.v1` | 3 | 1h | agent event detector | none |
| 37 | `agents.analysis-requests.v1` | 3 | 7d | Backend API | `gops-agent-analysis-worker` |
| 38 | `agents.deep-analysis-requests.v1` | 3 | 7d | analysis worker when enabled | `gops-agent-deep-analysis-worker` |
| 39 | `agents.analysis-results.v1` | 3 | 7d | analysis/deep worker or orchestrator | `gops-agent-delivery-gateway` |
| 40 | `agents.query-understanding-events.v1` | 3 | 7d | analysis/deep worker or orchestrator | none; audit topic |
| 41 | `agents.notification-decisions.v1` | 3 | 7d | analysis/deep worker or orchestrator | `gops-agent-notification-publisher` |
| 42 | `agents.dlq.v1` | 3 | 7d | reserved | none |
...
| `alfaka-raw-s3-archive` | 1 | all six raw topics | auto commit default |
...
| `alfaka-chart-derived-data-worker` | 2 | chart-derived requests | manual after processing |
...
The raw S3 archive uses the shared consumer default `enable_auto_commit=true`.
It buffers records before uploading them, so a crash can advance its Kafka
offset before the buffered S3 object is written. This does not remove the source
message from Kafka, but the same group resumes after the committed position and
can leave a raw archive gap. The processed S3 and ClickHouse paths explicitly
disable auto commit and commit only after their side effect succeeds.

## Retention Overrides

| Topic | Retention | Segment time | Segment size |
| --- | ---: | ---: | ---: |
| `market.layer.candles.live.v1` | 30 minutes | 5 minutes | 128 MiB |
...
`infra/k8s/base/platform/kafka/topics.txt`, the live EKS cluster, and the local
topic creation script include the two chart-derived topics. The nominal
`platform/kafka/topics.txt` file currently omits them. Until that file is
corrected, the in-cluster topic file is the accurate 42-topic deployment list.
