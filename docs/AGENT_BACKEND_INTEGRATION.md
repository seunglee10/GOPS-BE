...
manifest에 정의된 단계 ID만 받아, 운영자가 시장 조망·추천·기업 분석·차트 분석·
예약매매·본장·이벤트·마감 흐름을 대기 없이 이동하게 한다.

기본 `saturday-demo-amd-iff-oke` 시나리오는 AMD/IFF/OKE의 합성 trade와 quote를
...
시연 동안만 사용하며 live 경로 복구 뒤 실제 시세가 다시 덮어쓴다.
