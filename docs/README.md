...
사용자는 종목 상세 화면에서 실시간 차트와 관련 정보를 확인하고, 자연어 또는 직접 조작으로 차트의 종목, 기간, 비교 대상, 보조지표를 변경한다. AI 응답은 전역 auto toggle이 꺼져 있으면 사용자가 검토하고 승인할 수 있는 제안으로 다루고, 켜져 있으면 검증된 command 묶음으로 적용하되 undo/redo 가능한 변경으로 기록한다.
