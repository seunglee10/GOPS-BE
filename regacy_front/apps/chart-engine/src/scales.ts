...
export function createTimeScale({
  ...
  const safeVisibleSlotCount = Math.max(1, Math.round(visibleSlotCount ?? visibleCandles.length));
  const safeViewportStartIndex = typeof viewportStartIndex === "number" ? viewportStartIndex : visibleStartIndex;
  const safeViewportEndIndex = typeof viewportEndIndex === "number"
    ? viewportEndIndex
    : safeViewportStartIndex + safeVisibleSlotCount;
  const slotWidth = width / safeVisibleSlotCount;
  const candleCenter = (visibleIndex: number) => left + slotWidth * visibleIndex + slotWidth / 2;
  const logicalToX = (logicalIndex: number) => candleCenter(logicalIndex - safeViewportStartIndex);

  return {
    visibleStartIndex,
    visibleEndIndex,
    viewportStartIndex: safeViewportStartIndex,
    viewportEndIndex: safeViewportEndIndex,
    visibleSlotCount: safeVisibleSlotCount,
    slotWidth,
    candleCenter,
    logicalToX,
    xToLogical: (x: number) => safeViewportStartIndex + (x - left) / Math.max(0.0001, slotWidth),
    ...
    xToNearestCandle: (x: number) => {
      const logicalIndex = Math.round(safeViewportStartIndex + (x - left - slotWidth / 2) / Math.max(0.0001, slotWidth));
      if (logicalIndex < visibleStartIndex || logicalIndex >= visibleEndIndex) {
        return undefined;
      }
      return candles[logicalIndex];
    }
  };
}
...
export function createCoordinateTransform(scene: Pick<RenderScene, "allCandles" | "candles" | "document" | "plot" | "visibleStartIndex" | "visibleEndIndex" | "viewportStartIndex" | "viewportEndIndex" | "visibleSlotCount" | "scales">): CoordinateTransform {
  ...
  const timeScale = createTimeScale({
    ...
    viewportStartIndex: scene.viewportStartIndex,
    viewportEndIndex: scene.viewportEndIndex,
