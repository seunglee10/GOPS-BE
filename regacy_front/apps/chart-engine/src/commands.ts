...
import { clampFutureRightOffset } from "./viewport";
import type {
  ChartCommand,
  ChartCommandActor,
...
function applyDocumentMutation(document: ChartDocument, command: ChartCommand): string | null {
  switch (command.type) {
    ...
    case "chart.viewport.set": {
      ...
      const nextVisibleCount = visibleCount === null
        ? document.viewport.visibleCount
        : clamp(Math.round(visibleCount), 12, maxRequestBarsForInterval(document.timeframe));
      document.viewport = {
        visibleCount: nextVisibleCount,
        rightOffset: rightOffset === null ? document.viewport.rightOffset : clampFutureRightOffset(rightOffset, nextVisibleCount)
      };
      return null;
    }
