...
class AgentOrchestrator:
    def __init__(self, store: InMemoryReportStore | None = None):
        ...
        self.answer_composer = FinalAnswerSynthesizer()
    ...
    def _build_workflow(self):
        ...
        try:
            ...
            graph.add_edge("route_intent", "run_analysis_agents")
            graph.add_edge("run_analysis_agents", "compose_answer")
            graph.add_edge("compose_answer", "propose_layout")
    ...
    def _route_intent(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if not selected_roles:
            ...
            selected_roles = [role for role in MVP_ANALYSIS_ROLES if role in requested_roles]
    ...
    def _run_analysis_agents(self, state: dict[str, Any]) -> dict[str, Any]:
        selected_roles = [
            role
            for role in MVP_ANALYSIS_ROLES
        ...
        if not selected_roles:
            selected_roles = list(MVP_ANALYSIS_ROLES)
        if len(selected_roles) == 1:
            role = selected_roles[0]
            role_findings = [self._safe_analyze_role(role, agents[role], state["context"])]
            provider_evidence = collect_provider_evidence(role_findings)
            state["context"].providerEvidence = provider_evidence
            return {**state, "selected_roles": selected_roles, "role_findings": role_findings, "provider_evidence": provider_evidence}
    ...
    def _compose_answer(self, state: dict[str, Any]) -> dict[str, Any]:
        request = state["request"]
        symbol = state["symbol"]
        intent = state["intent"]
        events = state["events"]
        ...
        final_answer = self.answer_composer.synthesize(
            symbol=symbol,
            intent=intent,
            route=route,
    ...
    def _propose_layout(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        layout = self.layout_agent.propose(
            context,
...
def collect_provider_evidence(findings) -> list[EvidenceItem]:
    ...
    for finding in findings:
        evidence.extend(item for item in finding.evidence if item.provider in {"news", "ontology"})
...
def resolve_requested_roles(agent_ids: Any) -> set[str]:
    all_roles = set(MVP_ANALYSIS_ROLES)
...
MVP_ANALYSIS_ROLES = ("chart", "news", "ontology")
