...
class AgentOrchestrator:
    ...
    def analyze(self, request: dict[str, Any]) -> AnalysisReport:
        ...
        summary = build_summary(symbol, role_findings, events)
        report = AnalysisReport(
            ...
            summary=summary,
            rationale="Role findings were composed by the agent-orchestrator v1 skeleton.",
...
def build_summary(symbol: str, findings, events: list[MarketEvent]) -> str:
    if events:
        ...
        return f"{symbol} has a {strongest.severity} {strongest.eventType} signal; provider adapters remain empty in v1."
    return f"{symbol} multi-agent analysis completed with chart evidence and empty external provider adapters."
