...
def compile_rule_layers_v2(*, symbol: str, interval: str, features: dict[str, Any], candles: list[dict[str, Any]], generated_at: str, higher_assets: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    for event in events:
        if len(structure_drawings) >= 3 or sum(item["type"] == "flagMarker" for item in structure_drawings) >= 1: break
        ...
        structure_drawings.append(drawing); structure_selected.append({"candidateId": candidate_id, "drawingIds": [drawing["id"]], "evidenceRefs": [candidate_id, *event.get("refIds", [])], "quality": {"score": .8 if event.get("currentImpact") == "high" else .65, "ageBars": event.get("ageBars"), "currentImpact": event.get("currentImpact"), "eventState": (event.get("detail") or {}).get("state")}})
