...
class GraphDBOntologyProvider(OntologyProvider):
    ...
    def fetch(self, request: ProviderRequest) -> list[EvidenceItem]:
        ...
        try:
            ...
            # GraphDB connectivity failures are not cached, so the next request retries immediately.
            return [
                EvidenceItem(
                    provider="ontology",
                    status="no-data",
        ...
        primary_rows = [*per_symbol_rows[primary_symbol]["theme"], *per_symbol_rows[primary_symbol]["control"], *theme_matched_rows]
