...
class PostgresRecommendationRepository(RecommendationRepository):
            ...
            with self._connect() as conn:
                row = conn.execute(
                    ...
                    INSERT INTO stock_recommendation_runs (
                        user_sub, run_key, slot_start, slot_start_ts,
                        market_date, market_date_value, status,
                        profile_snapshot, market_snapshot_time, market_snapshot_at, summary,
                        ...
                        evidence_snapshot_id, model_version, generated_at
                    )
                    VALUES (
                        %s, %s, %s, %s::timestamptz,
                        %s, %s::date, %s,
                    ...
                    SET status = EXCLUDED.status,
                        ...
                        market_date_value = EXCLUDED.market_date_value,
    ...
    def _run_with_items(self, conn: psycopg.Connection, run: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = _json_ready(_prefer_typed_run_fields(run))
...
def _json_ready(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        internal_columns = {
            "app_user_id", "instrument_id", "slot_start_ts", "market_date_value",
            "market_snapshot_at", "model_version",
        }
        return {key: _json_ready(item) for key, item in value.items() if key not in internal_columns}
    if isinstance(value, list):
        return [_json_ready(item) for item in value]
    return value


def _prefer_typed_run_fields(run: dict[str, Any]) -> dict[str, Any]:
    preferred = dict(run)
    mappings = (
        ("slot_start_ts", "slot_start"),
        ("market_date_value", "market_date"),
        ("market_snapshot_at", "market_snapshot_time"),
    )
    for typed_name, public_name in mappings:
        typed_value = preferred.get(typed_name)
        if typed_value is not None:
            if isinstance(typed_value, datetime):
                typed_value = typed_value.astimezone(timezone.utc)
            preferred[public_name] = typed_value.isoformat()
    return preferred
