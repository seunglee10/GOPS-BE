...
from market_data.serving.intervals import candle_count_for_24h  # noqa: E402
from market_data.serving.hot_symbols import DEFAULT_HOT_LIMIT, build_hot_symbols_payload  # noqa: E402
from market_data.serving.provider import MarketDataProvider  # noqa: E402
from market_data.serving.time_utils import parse_utc_time  # noqa: E402
...
def watchlist_summaries(
    ...
    if symbols is not None:
        ...
        requested_symbols = read_watchlist_symbols(user_id, legacy_user_id=legacy_user_id)
...
def replace_watchlist_symbols(
    ...
    try:
        ...
        RealtimeSubscriptionCohortService(redis_client, auto_reconcile=False).replace_user_watchlist(
            legacy_user_id or user_id,
            requested_symbols,
        )
        if legacy_user_id:
            _write_v2_watchlist(redis_client, user_id, requested_symbols)
    except ValueError as exc:
...
def read_watchlist_symbols(user_id: str | None = None, legacy_user_id: str | None = None) -> list[str]:
    provider = get_market_data_provider()
    if user_id:
        try:
            redis_client = provider.redis_provider.redis
            if legacy_user_id:
                v2_symbols, migrated = _read_v2_watchlist(redis_client, user_id)
                if migrated:
                    return normalize_watchlist_symbol_list(v2_symbols, reject_outside=False)
                service = RealtimeSubscriptionCohortService(redis_client, auto_reconcile=False)
                legacy_symbols = normalize_watchlist_symbol_list(
                    service.user_watchlist_symbols(legacy_user_id),
                    reject_outside=False,
                )
                _write_v2_watchlist(redis_client, user_id, legacy_symbols)
...
def replace_portfolio_subscription_symbols(
    user_id: str,
    symbols: list[str],
    legacy_user_id: str | None = None,
) -> dict[str, Any]:
    ...
    try:
        redis_client = provider.redis_provider.redis
        normalized = RealtimeSubscriptionCohortService(redis_client, auto_reconcile=False).replace_user_portfolio(
            legacy_user_id or user_id,
            symbols,
        )
        if legacy_user_id:
            _write_v2_portfolio(redis_client, user_id, normalized)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
...
def _write_v2_watchlist(redis_client: Any, app_user_id: str, symbols: list[str]) -> None:
    keys = RedisKeyBuilder()
    set_key = keys.user_v2_watchlist_symbols(app_user_id)
    order_key = keys.user_v2_watchlist_symbol_order(app_user_id)
    redis_client.delete(set_key)
    redis_client.delete(order_key)
    if symbols:
        redis_client.sadd(set_key, *symbols)
        redis_client.rpush(order_key, *symbols)
    redis_client.set(keys.user_v2_migration_marker(app_user_id, "watchlist"), "1")


def _read_v2_watchlist(redis_client: Any, app_user_id: str) -> tuple[list[str], bool]:
    keys = RedisKeyBuilder()
    marker = redis_client.get(keys.user_v2_migration_marker(app_user_id, "watchlist"))
    if not marker:
        return [], False
    ordered = redis_client.lrange(keys.user_v2_watchlist_symbol_order(app_user_id), 0, -1)
    if ordered:
        return list(ordered), True
    return sorted(redis_client.smembers(keys.user_v2_watchlist_symbols(app_user_id)) or []), True


def _write_v2_portfolio(redis_client: Any, app_user_id: str, symbols: list[str]) -> None:
    keys = RedisKeyBuilder()
    symbols_key = keys.user_v2_portfolio_symbols(app_user_id)
    redis_client.delete(symbols_key)
    if symbols:
        redis_client.sadd(symbols_key, *symbols)
    redis_client.set(keys.user_v2_migration_marker(app_user_id, "portfolio"), "1")
...
def resolve_latest_trade_price(symbol: str) -> dict[str, Any]:
    normalized_symbol = normalize_market_symbol(symbol)
    provider = get_market_data_provider()
    latest_price = _safe_latest_price(provider, normalized_symbol)
    price = _read_float(latest_price.get("price"))
    return {
        "symbol": normalized_symbol,
        "price": price,
        "timestamp": latest_price.get("timestamp") or latest_price.get("eventTime"),
        "sourceEventId": latest_price.get("sourceEventId"),
    }
