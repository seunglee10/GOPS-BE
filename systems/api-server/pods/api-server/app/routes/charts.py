...
from market_data.serving.intervals import MAX_CHART_CANDLE_LIMIT
...
def chart_active_symbol_heartbeat(
    ...
    user_id = (user.app_user_id or user.sub) if user else "anonymous"
...
def chart_watchlist(
    ...
    return watchlist_summaries(
        requested,
        user_id=user.app_user_id or user.sub,
        legacy_user_id=user.sub if user.app_user_id else None,
    )
...
def chart_watchlist_replace(body: SymbolListRequestBody, user: AuthenticatedUser = Depends(require_current_user)) -> dict[str, Any]:
    return replace_watchlist_symbols(
        user.app_user_id or user.sub,
        body.symbols,
        legacy_user_id=user.sub if user.app_user_id else None,
    )
...
def chart_portfolio_subscription_cohort(body: SymbolListRequestBody, user: AuthenticatedUser = Depends(require_current_user)) -> dict[str, Any]:
    return replace_portfolio_subscription_symbols(
        user.app_user_id or user.sub,
        body.symbols,
        legacy_user_id=user.sub if user.app_user_id else None,
    )
