...
(
    symbol LowCardinality(String), instrument_id Nullable(UUID),
    schema_version LowCardinality(String) DEFAULT 'company-journal.v2',
    analysis_as_of Date, generated_at DateTime64(3, 'UTC'),
...
(
    request_id String, symbol LowCardinality(String), instrument_id Nullable(UUID),
    schema_version LowCardinality(String) DEFAULT 'company-journal.v2',
    analysis_as_of Date, input_digest String,
...
ALTER TABLE company_journal_reports_v1 ADD COLUMN IF NOT EXISTS instrument_id Nullable(UUID) AFTER symbol;
ALTER TABLE company_journal_reports_v1 ADD COLUMN IF NOT EXISTS schema_version LowCardinality(String) DEFAULT 'company-journal.v2' AFTER instrument_id;
ALTER TABLE company_journal_generation_events_v1 ADD COLUMN IF NOT EXISTS instrument_id Nullable(UUID) AFTER symbol;
ALTER TABLE company_journal_generation_events_v1 ADD COLUMN IF NOT EXISTS schema_version LowCardinality(String) DEFAULT 'company-journal.v2' AFTER instrument_id;

CREATE OR REPLACE VIEW company_journal_reports AS SELECT * FROM company_journal_reports_v1;
CREATE OR REPLACE VIEW company_journal_generation_events AS SELECT * FROM company_journal_generation_events_v1;
