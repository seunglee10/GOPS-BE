...
class CompanyJournalRepository:
    ...
    def latest_verified(self, symbol: str) -> dict[str, Any] | None:
        rows = self.client.query_json_each_row(
            f"""
            SELECT *
            FROM {self.database}.company_journal_reports
    ...
    def load_source_bundle(self, symbol: str, cutoff: datetime | None = None) -> dict[str, Any]:
        source_suffix = "" if cutoff is not None else "_latest"
        ...
        company = self._one(
            f"SELECT symbol, company_name, cik, exchange, updated_at FROM {self.database}.sec_company_tickers_latest "
            "WHERE symbol = {symbol:String} ORDER BY updated_at DESC LIMIT 1 FORMAT JSONEachRow",
            {"symbol": symbol},
        ) or self._one(
            f"SELECT symbol, name AS company_name, exchange, updated_at FROM {self.database}.symbols_latest "
            "WHERE symbol = {symbol:String} ORDER BY updated_at DESC LIMIT 1 FORMAT JSONEachRow",
            {"symbol": symbol},
        )
        prices = self._rows(
            ...
            FROM {self.database}.chart_candles{source_suffix}
        ...
        news = self._rows(
            ...
            FROM {self.database}.news_company_daily_summaries{source_suffix}
            WHERE symbol = {{symbol:String}}
        ...
        metrics = self._rows(
            ...
            FROM {self.database}.sec_derived_metrics{source_suffix}
            WHERE symbol = {{symbol:String}}
              {versioned_dated_source_cutoff}
        ...
        filings = self._rows(
            ...
            FROM {self.database}.sec_filing_events{source_suffix}
            WHERE symbol = {{symbol:String}}
              {dated_source_cutoff}
        ...
        earnings_actuals = self._safe_rows(
            ...
            FROM {self.database}.sec_financial_facts{source_suffix}
            WHERE symbol = {{symbol:String}}
        ...
        earnings_estimates = self._safe_rows(
            ...
            FROM {self.database}.yahoo_earnings_estimates{source_suffix}
            WHERE symbol = {{symbol:String}}
              AND metric IN ('eps', 'revenue')
              AND fiscal_period IN ('Q1', 'Q2', 'Q3', 'Q4')
        ...
        graph = self._safe_one(
            ...
            FROM {self.database}.agent_graph_expansions{source_suffix}
            WHERE symbol = {{symbol:String}}
              {generated_source_cutoff}
    ...
    def load_analyst_summary(
        ...
        if cutoff is None:
            return self._safe_one(
                ...
                FROM {self.database}.yahoo_analyst_summaries_latest
                WHERE symbol = {{symbol:String}}
                  AND statement != ''
    ...
    def request_is_active(self, request_id: str) -> bool:
        row = self._one(
            ...
            FROM {self.database}.company_journal_generation_events
    ...
    def append_request_event(self, request: GenerationRequest, status: str, error: str | None = None) -> None:
        self.client.insert_json_each_row("company_journal_generation_events_v1", [{
            ...
            "instrument_id": instrument_id_for_symbol(request.symbol),
            "schema_version": "company-journal.v2",
    ...
    def daily_candidates(self, limit: int) -> list[str]:
        rows = self._rows(
            ...
            FROM {self.database}.chart_candles_latest
