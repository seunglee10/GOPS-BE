...
class RedisKeyBuilder:
    ...
    def user_v2_watchlist_symbols(self, app_user_id):
        return self.key(f"user:v2:{app_user_id}:watchlist:symbols")

    def user_v2_watchlist_symbol_order(self, app_user_id):
        return self.key(f"user:v2:{app_user_id}:watchlist:symbol-order")

    def user_v2_portfolio_symbols(self, app_user_id):
        return self.key(f"user:v2:{app_user_id}:portfolio:symbols")

    def user_v2_migration_marker(self, app_user_id, namespace):
        return self.key(f"user:v2:{app_user_id}:migration:{namespace}")
