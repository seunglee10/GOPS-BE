...
from market_data.common.kafka_topics import closed_candle_topic_values
...
from market_data.serving.session_buckets import bucket_policy_for_candle
from market_data.storage.candle_validation import invalid_candle_reason


INSTRUMENT_LINKED_TABLES = frozenset({
    "simulation_replay_staging", "simulation_replay_events", "simulation_replay_candles_1m",
    "trade_ticks", "quote_ticks", "chart_candles", "market_status_events", "market_events",
    "symbols", "agent_graph_expansions", "news_articles", "news_article_localizations",
    "news_company_daily_summaries", "backfill_jobs", "order_flow_profile_daily",
    "chart_analysis_assets", "sec_company_tickers", "sec_filing_events", "sec_raw_artifacts",
    "sec_financial_facts", "sec_derived_metrics", "sec_frames", "yahoo_earnings_estimates",
    "yahoo_analyst_summaries", "company_journal_reports_v1",
    "company_journal_generation_events_v1",
})
...
class ClickHouseHttpClient:
    ...
    def insert_json_each_row(self, table, rows, deduplication_token=None):
        ...
        if table in INSTRUMENT_LINKED_TABLES:
            rows = [
                {
                    **row,
                    "instrument_id": row.get("instrument_id") or canonical_instrument_id(row.get("symbol")),
                }
                for row in rows
            ]
