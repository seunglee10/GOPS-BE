...
from market_data.common.canonical import CANONICAL_VERSION, HISTORICAL_SERVING_PRICE_ADJUSTMENTS, SERVING_PRICE_ADJUSTMENTS
from market_data.common.symbols import is_crypto_symbol
from market_data.serving.dto import snapshot
from market_data.serving.intervals import INTRADAY_DERIVED_INTERVALS, INTRADAY_INTERVAL_MINUTES, historical_source_interval_for, max_request_bars, normalize_chart_interval, resolve_candle_limit
...
class ClickHouseMarketDataProvider:
    ...
    def order_flow_daily_profiles(self, symbol, from_date, to_date, limit=100000):
        ...
        SELECT
          ...
          volume
        FROM {self.table('order_flow_profile_daily_latest')}
...
def with_higher_timeframe_closed_state(rows, interval, *, now=None):
    ...
    if normalized_interval not in {"1W", "1M"}:
        return list(rows or [])
    from market_data.analytics.analysis_candles import is_analysis_candle_bucket_complete
