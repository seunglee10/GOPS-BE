...
>
> Use `../docs/CHART_DATA_REBUILD_PLAN.md` for active reset scope, Redis key
> families, S3 prefixes, and SIP/BOATS feed rules. This runbook is historical
> parity context and must not override the on-demand rebuild contract.
...
- S3 raw/final/manifest prefixes must keep their roles separate. Raw objects are
  backup/audit only and are not chart-serving or materialization inputs. Live
  candles and quotes live in Redis/WebSocket state, not S3. ClickHouse serving
  should be rebuilt from final canonical parquet/manifest or from bounded
...
- `/api/charts/hot-symbols` returns Hot Top10 from active subscription candidates.
