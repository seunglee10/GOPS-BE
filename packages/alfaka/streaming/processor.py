...
from alfaka.streaming.transforms import CandleAggregator, LiveCandleBuilder, MovingAverageState, normalize_bar, normalize_trade
...
def main():
    ...
    raw_topics = [f"{raw_prefix}.bars", f"{raw_prefix}.updated-bars", f"{raw_prefix}.trades"]
    ...
    print(f"Processed Topics: {ticks_topic}, {live_candle_topic}, {closed_candle_topic}", flush=True)
    ...
    for record in consumer:
        ...
        if channel == "trades":
            ...
            write_trade_to_redis(redis_client, trade)
            write_live_candle_to_redis(redis_client, live_candle)
            publish_chart_event(redis_client, websocket_event("LIVE_CANDLE_UPDATE", trade["symbol"], "1m", live_candle, feed=trade.get("feed") or "unknown"))
            continue
        ...
        if channel in {"bars", "updatedBars"}:
            ...
            write_closed_candle_to_redis(redis_client, candle_1m)
            publish_chart_event(redis_client, websocket_event(event_type, candle_1m["symbol"], "1m", candle_1m, feed=candle_1m.get("feed") or "unknown"))
            ...
            for interval_minutes in (5, 10):
                ...
                if aggregated:
                    ...
                    write_closed_candle_to_redis(redis_client, aggregated)
                    publish_chart_event(redis_client, websocket_event("CANDLE_CLOSED", aggregated["symbol"], aggregated["interval"], aggregated, feed=aggregated.get("feed") or "unknown"))
            continue
...
def write_trade_to_redis(redis_client, trade):
    key = f"price:{trade['symbol']}:latest"
...
def write_live_candle_to_redis(redis_client, candle):
    key = f"candle:{candle['symbol']}:1m:live"
    redis_client.set(key, json.dumps(candle, ensure_ascii=False, separators=(",", ":")))
    redis_client.expire(key, 86400)


def write_closed_candle_to_redis(redis_client, candle):
    latest_key = f"candle:{candle['symbol']}:{candle['interval']}:latest"
    series_key = f"candles:{candle['symbol']}:{candle['interval']}"
...
def publish_chart_event(redis_client, event):
    event_json = json.dumps(event, ensure_ascii=False, separators=(",", ":"))
    redis_client.publish(f"market.events:{event['symbol']}", event_json)
    redis_client.publish("market.events", event_json)
