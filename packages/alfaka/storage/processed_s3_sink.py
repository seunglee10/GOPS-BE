...
# 출력: market-data/processed 아래 Parquet 또는 JSONL 파일.
import io
import json
import os
import sys
...
def main():
    ...
    s3_prefix = os.getenv("S3_PROCESSED_PREFIX", "market-data/processed")
    ...
    print(f"S3 저장 위치: s3://{s3_bucket}/{s3_prefix}, format={output_format}", flush=True)

    for record in consumer:
        payload = record.value
        partition_key = s3_partition_key(s3_prefix, payload)
...
def s3_partition_key(prefix, payload):
    ...
    return f"{prefix}/trades/symbol={symbol}/year={event_time:%Y}/month={event_time:%m}/day={event_time:%d}/hour={event_time:%H}"
